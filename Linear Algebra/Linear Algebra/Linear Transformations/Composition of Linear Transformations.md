...
For the given [[Linear Transformation]] $\mathcal{F : V \rightarrow W,G : X \rightarrow V}$, we can show that their composition, $\mathcal{F \circ G} : \mathcal{X} \rightarrow \mathcal{W}$ is itself another linear transformation for the element $\vec{x},\vec{y}\in \mathcal{X}$. Then we can show that:
